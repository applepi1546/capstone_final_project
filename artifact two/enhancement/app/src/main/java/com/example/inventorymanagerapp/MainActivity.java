    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        buttonSignUp.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v) // Sign up button
            {
                ...
                else if (loginDB.checkUser(username, password)) // Check user exists
                {
                    Toast.makeText(MainActivity.this, "User already exists", Toast.LENGTH_SHORT).show();
                    return;
                }
                else if (password.length() < 8) // Password length check
                {
                    Toast.makeText(MainActivity.this, "Password must be at least 8 characters", Toast.LENGTH_SHORT).show();
                    return;
                }
                else if (!password.matches(".*[!@#$%^&*()-+=<>?/{}~].*")) // Password special character
                {
                    Toast.makeText(MainActivity.this, "Password must contain at least one special character", Toast.LENGTH_SHORT).show();
                    return;
                }
                if (!username.matches("[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")) // check if valid email
                {
