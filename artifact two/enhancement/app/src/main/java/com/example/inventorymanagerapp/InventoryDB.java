...
public class InventoryDB extends SQLiteOpenHelper  // Inventory Database
{
    ...
    public Cursor filterTable(String username, String query)  // filter table
    {
        SQLiteDatabase db = this.getReadableDatabase(); // get db
        String searchQuery = "%" + query + "%"; // *query*
        return db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_USER + " = ? AND " + COLUMN_ITEM_NAME + " LIKE ?",
                new String[]{username, searchQuery}); // return the query
    }

    public Cursor sortInventory(String username, String sortColumn, boolean ascending) {
        SQLiteDatabase db = this.getReadableDatabase(); // get db

        String sortOrder = ascending ? "ASC" : "DESC"; // sort order
        String query = "SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_USER + " = ?" + " ORDER BY " + sortColumn + " " + sortOrder; // query

        return db.rawQuery(query, new String[]{username}); // return the query
    }
